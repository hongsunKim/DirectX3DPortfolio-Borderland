...
//버택스 진입
VS_OUTPUT_RECIVESHADOW vs_OUTLINE( VS_INPUT Input, uniform int iNumBones )
{
   VS_OUTPUT_RECIVESHADOW Output = (VS_OUTPUT_RECIVESHADOW)0;
   ...
   worldPos.xyz = worldPos.xyz+Output.Normal.xyz*0.4f;
   Output.Position = mul( worldPos, matViewProjection );
   Output.viewDir = vEyePos.xyz - worldPos.xyz;
   Output.worldPos = worldPos;


   Output.FinalPos = worldPos;		//변환 정보
   Output.LightClipPos = mul( worldPos, matLightViewProjection );	//광원 입장에서 본 위치
...
VertexShader vsOutline[ 4 ] = { compile vs_3_0 vs_OUTLINE( 1 ),
...
technique Base
{
    ...
    {
	CULLMODE = CW;
	VertexShader = (vsOutline[ CurNumBones ]);
	PixelShader = compile ps_3_0 ps_main_outline();
    }
}
...
technique ReciveShadowToonMask
{
	pass Pass_0
	{
		VertexShader = (ReciveShadowArr[CurNumBones]);
		PixelShader = compile ps_3_0 ps_ReciveShadowToonMask();
	}
	pass Pass_1
    	{
		CULLMODE = CW;
...
technique ReciveShadowToonMaskSkag
{
	pass Pass_0
	{
		VertexShader = (ReciveShadowArr[CurNumBones]);
		PixelShader = compile ps_3_0 ps_ReciveShadowToonMaskSkag();
	}
	pass Pass_1
    	{
		CULLMODE = CW;
