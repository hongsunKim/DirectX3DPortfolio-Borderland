...
void cXMesh_Skinned::Render(const cTransform* pTrans)
{
	...
	//m_pSkinnedEffect->SetTechnique("ReciveShadowToon");
	...
	if (pBone->pMeshContainer)
	{
		...
		if (pBoneMesh->BufBoneCombos != NULL)
		{
			...
			for (DWORD i = 0; i < pBoneMesh->NumAttributesGroup; i++)
			{
				...
				m_pSkinnedEffect->SetTexture("Mask_Tex", pBoneMesh->vecMaskTex[materialIndex]);
				m_pSkinnedEffect->SetTexture("Color_Tex", pBoneMesh->vecColorTex[materialIndex]);
