...
)
{
	...
	for (DWORD i = 0; i < NumMaterials; i++)
	{
		...
		if (pMaterials[i].pTextureFilename != NULL)
		{
			...
			// mask 맵은 _M 이 붙는다.
			//
			texFilePath = texFile + "_M." + texExp;

			LPDIRECT3DTEXTURE9 pMaskTex =
				RESOURCE_TEXTURE->GetResource(texFilePath);
			//만약 Texture 가 없다면...
			if (pMaskTex == NULL) {
				pMaskTex = RESOURCE_TEXTURE->GetResource("../../Resources/Meshes/specularDefault.png");
			}
			...
			// Color 맵은 _C 이 붙는다.
			//
			texFilePath = texFile + "_C." + texExp;

			LPDIRECT3DTEXTURE9 pColorTex =
				RESOURCE_TEXTURE->GetResource(texFilePath);
			//만약 Texture 가 없다면...
			if (pColorTex == NULL) {
				pColorTex = RESOURCE_TEXTURE->GetResource("../../Resources/Meshes/specularDefault.png");
			}
			...
			boneMesh->vecColorTex.push_back(pMaskTex);
			boneMesh->vecMaskTex.push_back(pColorTex);
		}
		...
		else
		{
			...
			boneMesh->vecColorTex.push_back(NULL);
			boneMesh->vecMaskTex.push_back(NULL);
		}
	}
