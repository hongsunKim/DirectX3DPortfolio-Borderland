...
void cTransform::ShakePos(D3DXVECTOR3 pos)
{
	this->position2.x = pos.x;
	this->position2.y = pos.y;
	this->position2.z = pos.z;
}
...
void cTransform::ShakeUpdate(float timeDelta)
{
	...
	if (this->ShakePosPower > 0.0f && this->ShakePosFlag != 0)
	{
		...
		D3DXVECTOR3 localPos = position2;
	...
	//	D3DXVECTOR3 localRot(0, 0, 0);
		...
		D3DXVECTOR3 localRot = position2;
...
void cTransform::BodyControl()
{
	...
	static float sensitivityH = 0.5;					//가로 민감도
	...
	//앵글값을 min max 범위 안으로
	//nowAngleH = Clamp(nowAngleH, minAngleH, maxAngleH);

	if (nowAngleH >= 360.f)
	{
		nowAngleH -= 360.f;
	}
	else if (nowAngleH < 0.f)
	{
		nowAngleH += 360.f;
	}
	...
	//다시 마우스 위치를 센터로...
	SetMousePos(screenCenterX, mousePos.y);

	cTransform tempTrans;
	tempTrans.SetRotateWorld(0.f, nowAngleH * ONE_RAD, 0.0f);
	RotateSlerp(*this, tempTrans, 0.3);

	//this->SetRotateWorld(0.f, nowAngleH * ONE_RAD, 0.0f);
}



void cTransform::UpperBodyControl(cTransform* bodyTrans)
{
	//디폴트 컨트롤을 위한 카메라 Angle 값
	//static float nowAngleH = 0.0f;			//수평앵글
	static float nowAngleV = 0.0f;			//수직앵글
	...
	static float minAngleV = -85.0f;			//수직 최저 앵글
												//static float sensitivityH = 1.0f;					//가로 민감도
	static float sensitivityV = 0.5;					//세로 민감도 ( 이값이 음수면 Invert Mouse )
	static D3DXVECTOR3 nowVelocity(0, 0, 0);			//현제 방향과 속도를 가진 벡터
	...
	//다시 마우스 위치를 센터로...
	SetMousePos(mousePos.x, screenCenterY);

	D3DXMATRIXA16 temp = this->GetWorldRotateMatrix();
	D3DXMatrixRotationAxis(&temp, &bodyTrans->GetRight(), nowAngleV * ONE_RAD);

	cTransform tempTrans;
	tempTrans.SetWorldMatrix(temp);
	RotateSlerp(*this, tempTrans, 0.3);
	SetWorldMatrix(temp);
	//this->SetRotateLocal(nowAngleV * ONE_RAD, 0.f, 0.0f);
}

void cTransform::FPSGameCameraControl()
{
	//디폴트 컨트롤을 위한 카메라 Angle 값
	static float nowAngleH = 0.0f;			//수평앵글
	static float nowAngleV = 0.0f;			//수직앵글
...
//속도를 유지하며 해당 크기만큼 이동(높이값은 유지된다)
void cTransform::MovePositionFPS(float dx, float dz)
{
	D3DXVECTOR3 move(0, 0, 0);
	...
	for (int i = 0; i < 3; ++i)
	{
		moveAxis[i] = D3DXVECTOR3(moveAxis[i].x, 0, moveAxis[i].z);
		D3DXVec3Normalize(&moveAxis[i], &moveAxis[i]);
	}
