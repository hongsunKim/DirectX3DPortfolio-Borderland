...
bool cSkinnedAnimation::IsPlay(std::string aniName)
{
	if (m_bPlay)
	{
		if (m_pNowPlayAnimationSet == m_mapAnimSet.find(aniName)->second)
			return true;
	}
...
bool cSkinnedAnimation::ReachPoint(float f)
{
	//0 번 Track 정보를 얻는다
	m_pAnimController->GetTrackDesc(0, &m_Track_Desc_0);
