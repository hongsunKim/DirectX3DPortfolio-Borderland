...
HRESULT enemyClass1::Init(float scale /*1.f*/, float followRange/*32.f*/, float airAttRange/*16.f*/, float enemyRange/*4.f*/, int SKAG_MODE, D3DXVECTOR3 EnemyPos,
	vector<cSetBoundObject*> pVecObjects, cPlayer* player, cTerrain* Terrain)
{
	...
	cXMesh_Skinned* pSkinned = RESOURCE_SKINNEDXMESH->
		GetResource("../../Resources/Enemy/skag.X",
			&matCorrection);
	...
	//this->vShadow_Enemy[0]->pSkinned = vShadow_Enemy[0]->pSkinned;
	//
	...
	this->vShadow_Enemy[0]->pSkinned->AddApplyTransform("Bip01", &_enemyTrans);
...
void enemyClass1::Release()
{
	this->vShadow_Enemy[0]->pSkinned->Release();
}
void enemyClass1::Update(float timeDelta, cBoundSphere *playerSphere, cBoundSphere *playerCoreSphere, bool playerAir, D3DXVECTOR3 playerXYZ)
{
	...
	//tempPlayer.SetWorldPosition(playerXYZ);

	//tempPlayer.SetWorldPosition(xyz);
	...
	if (_Enemy_Stat.Hp <= 0 && _Enemy_Stat.LiveOrDie)
	{
		...
		{
			this->vShadow_Enemy[0]->pSkinned->PlayOneShotAfterHold("Anims_27", 0.3f);
		}
		//if (vShadow_Enemy[0]->pSkinned->getPlay())
		{
			_Enemy_Stat.LiveOrDie = false;
			SOUNDMANAGER->play("스캐그 죽는소리", 1.0F);
		}
	...
	if (_Enemy_Stat.Hp > 0)
	{
		...
		//if (_enemyMode == ENEMY_MODE_RUN_STAY && _prevEnemyMode == ENEMY_MODE_JUMP_SUCCESS)
		//{
		//
		//}
...
void enemyClass1::Render(cCamera* pMainCamera , cLight_Direction* Light_Dt)
{
	if (_Enemy_Hit) {
		char str[128];
	...
	//	this->vShadow_Enemy[0]->pSkinned->Render(&_skinnedTrans);
	...
	if (Text_Check) {
		...
		{
			wsprintf(str, "공격 완료");
			DXFONT_MGR->PrintStyleTextOutline("차차차", str, WINSIZE_X - 100, 100, 0xff000000, 0xffff0000);
		}
		else
		{
			wsprintf(str, "공격 실패");
			DXFONT_MGR->PrintStyleTextOutline("차차차", str, WINSIZE_X - 100, 200, 0xff000000, 0xffff0000);
		}
	}
...
void enemyClass1::airAttSet(cTransform *playerTrans, cBoundSphere *playerSphere, cBoundSphere *playerCoreSphere)
{
...
void enemyClass1::normalFollow(cTransform *playerTrans, cBoundSphere *playerSphere)
{
...
void enemyClass1::interpolation(cTransform *playerTrans)
{
...
void enemyClass1::DamageFlag(float Attak1, float Attak2, float Attak3)
{
	if (_damageFlag)
	{
		falseDamageFlag();
